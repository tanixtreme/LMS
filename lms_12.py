...
### Module to add books into books table. 

def add_book():
  conn = sqltor.connect(host="localhost", database="lms_12", user="root", password="root@123")
  cursor = conn.cursor()
  ...
  sql = 'insert into books(btitle,author,price,pages,publisher,edition,status) values ( "' + \
...
### Module to add members into member table. 
def add_member():
  conn = sqltor.connect(host="localhost", database="lms_12", user="root", password="root@123")
  cursor = conn.cursor()
  ...
  sql = 'insert into members(mname,class,address,phone,email) values ( "' + \
...
def modify_book():
  ...
  if field =='pages' or field == 'price':
      sql = 'update books set ' + field + ' = '+value+' where bid = '+book_id+';'
  else:
      sql = 'update books set ' + field + ' = "'+value+'" where bid = '+book_id+';'
...
def modify_member():
  ...
  value = input('Enter new value :')
  sql = 'update members set '+ field +' = "'+value+'" where mid = '+mem_id+';'
  print(sql)
  try:
      cursor.execute(sql)
      ...
      print('\n\nMember details unable to Update.....')
  conn.close()
  wait = input('\n\n\n Press any key to continue....')
...
def issue_book():
  ...
  if len(result1) == 0:
    if result == 'available':
        ...
        sql_book = 'update books set status="issue" where bid ='+book_id + ';'
        cursor.execute(sql)
...
def return_book():
    ...
    if result==None:
       ...
       sql='update books set status ="available" where bid ='+book_id +';'
...
def change_book_status(status,book_id):
    ...
    sql = 'update books set status = "'+status +'" where bid ='+book_id + ' and status ="available"'
    cursor.execute(sql)
...
########################################################################################
# This is Main Module - from menu other modules are being called. 
# Main Menu Module 
########################################################################################
def main_menu():
    while True:
